...
module.exports = (wixFilePath, config = {}) => {
  const content = createDocument()
    .root()
    .import(_defines(config))
    .import(_product())
...
const createDocument = _ => create({ version: '1.0' }).ele({
  Wix: {
...
const _defines = (config) => fragment().ins([
